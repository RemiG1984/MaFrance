...
router.get(
  ...
  (req, res, next) => {
    ...
    const pageLimit = limit ? Math.min(parseInt(limit), 100) : null;
    ...
    if (tag) {
      sql += " AND (tags = ? OR tags LIKE ? OR tags LIKE ? OR tags LIKE ? OR tags LIKE ? OR tags LIKE ?)";
...
// GET /api/francocides/stats - Get statistics
router.get(
  "/stats",
