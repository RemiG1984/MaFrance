(function () {
    ...
    if (!departementSelect || !communeInput || !communeList || !lieuxSelect || !resultsDiv || !articleListDiv || !filterButtonsDiv) {
        console.error('One or more DOM elements are missing');
        return;
    }

    // Initialize modules
    const locationHandler = LocationHandler(departementSelect, communeInput, communeList, lieuxSelect, resultsDiv, departmentNames);
    ...
    // Shared state
    let currentLieu = '';
    let allArticles = []; // Moved from articleHandler to main.js for shared access
    ...
    departementSelect.addEventListener('change', () => {
        ...
        currentLieu = '';
        articleHandler.setFilter(null); // Reset filter
    ...
    communeInput.addEventListener('change', async () => {
        ...
        if (departement && commune) {
            // Fetch COG code for the commune
            try {
                const response = await fetch(`/search?dept=${departement}&q=${encodeURIComponent(commune)}`);
                if (!response.ok) {
                    throw new Error(`Erreur lors de la récupération de la commune: ${response.statusText}`);
                }
