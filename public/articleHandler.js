const ArticleHandler = (function () {
    return function (articleListDiv, filterButtonsDiv) {
        ...
        function renderArticles(articles, lieu, filter = null) {
            ...
            if (lieu) {
                filteredArticles = filteredArticles.filter(article => 
                    article.lieu && 
                    article.lieu.toLowerCase().split(',').map(l => l.trim()).includes(lieu.toLowerCase())
                );
            ...
            if (filter) {
                filteredArticles = filteredArticles.filter(article => {
                    ...
                    console.log(`Checking article ${article.title} for ${filter}: ${value}, result: ${result}`);
                    return result;
                });
            }
        ...
        function renderFilterButtons(counts, articles, lieu) {
            ...
            const categories = [
                { name: 'Insécurité', key: 'insecurite', count: counts.insecurite || 0 },
                { name: 'Immigration', key: 'immigration', count: counts.immigration || 0 },
                { name: 'Islamisation', key: 'islamisme', count: counts.islamisme || 0 },
                { name: 'Défrancisation', key: 'defrancisation', count: counts.defrancisation || 0 },
