    ...
    <div class="results-section">
      ...
      <div v-else-if="correlationMatrix && correlationMatrix.length > 0" class="heatmap-section">
        ...
        <div class="heatmap-title">
          <h3>Matrice des corrélations entre métriques</h3>
          <p class="subtitle">
...
export default {
  ...
  setup() {
    ...
    const topCorrelations = computed(() => {
      if (!correlationMatrix.value.length || !metricLabels.value.length) return []
      const correlations = []
      
      for (let i = 0; i < correlationMatrix.value.length; i++) {
        for (let j = i + 1; j < correlationMatrix.value[i].length; j++) {
          const value = correlationMatrix.value[i][j]
          // Only show meaningful correlations that are not null
      ...
      for (let i = 0; i < metrics.length; i++) {
        ...
        for (let j = 0; j < metrics.length; j++) {
          if (i === j) {
            ...
            countRow.push(data.length)
          } else {
            // Implement pairwise deletion: only use rows where both values are valid
            ...
            if (validPairs.length < 20) {
              ...
              row.push(null)
              countRow.push(validPairs.length)
            } else {
        ...
        matrix.push(row)
        pairwiseCounts.push(countRow)
      }
      
      return { matrix, labels, pairwiseCounts }
    }
