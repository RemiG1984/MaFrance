...
router.get('/', (req, res) => {
    ...
    db.all(sql, params, (err, rows) => {
        if (err) {
            console.error('Database error in /api/qpv:', err);
...
router.get('/commune/:cog', (req, res) => {
    ...
    const sql = `
        ...
        FROM qpv_data
        WHERE COG = ?
        ORDER BY codeQPV ASC
    `;
    
    db.all(sql, [cog], (err, rows) => {
        if (err) {
            console.error('Database error in /api/qpv/commune:', err);
            return res.status(500).json({ 
                error: 'Database error', 
                details: err.message 
            });
...
router.get('/stats/departement/:dept', [validateDepartement], (req, res) => {
    ...
    db.get(sql, [dept], (err, row) => {
        if (err) {
            console.error('Database error in /api/qpv/stats/departement:', err);
            return res.status(500).json({ 
                error: 'Database error', 
                details: err.message 
            });
...
router.get('/search', (req, res) => {
    ...
    db.all(sql, params, (err, rows) => {
        if (err) {
            console.error('Database error in /api/qpv/search:', err);
            return res.status(500).json({ 
                error: 'Database error', 
                details: err.message 
            });
