...
router.get("/details", validateCOG, (req, res) => {
  const { cog } = req.query;
  db.get(
    'SELECT COG, departement, commune, population, logements_sociaux_pct, insecurite_score, immigration_score, islamisation_score, defrancisation_score, wokisme_score, number_of_mosques, mosque_p100k, total_qpv, pop_in_qpv_pct, total_places_migrants, places_migrants_p1k, Subventions_parHab FROM locations WHERE COG = ?',
    [cog],
    (err, row) => {
      if (err) return handleDbError(res, err);
