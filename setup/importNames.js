...
function importNames(db, callback) {
    ...
    function importCountryNames() {
        ...
        function readCountryNames() {
            return new Promise((resolve, reject) => {
                if (!fs.existsSync("setup/analyse_prenom_france.csv")) {
                    console.error(
                        "Erreur: analyse_prenom_france.csv n'existe pas dans le répertoire courant",
                    );
                    resolve(); // Continue with empty data
                    return;
                }

                fs.createReadStream("setup/analyse_prenom_france.csv")
                    .pipe(csv())
                    .on("data", (row) => {
                        const missingFields = [];
                        ...
                        if (missingFields.length > 0) {
                            console.warn(
                                `Ligne ignorée dans analyse_prenom_france.csv (champs manquants: ${missingFields.join(", ")}):`,
                                row,
                            );
                            return;
                        }
                    ...
                    .on("end", () => {
                        console.log(
                            `Lecture de analyse_prenom_france.csv terminée: ${countryRows} lignes`,
                        );
                        if (countryRows === 0) {
                            console.warn(
                                "Avertissement: analyse_prenom_france.csv est vide ou n'a pas de données valides",
                            );
                        }
                        resolve();
                    })
                    .on("error", (err) => {
                        console.error(
                            "Erreur lecture analyse_prenom_france.csv:",
                            err.message,
                        );
                        reject(err);
                    });
                ...
                db.serialize(() => {
                    ...
                    `,
                        (err) => {
                            if (err) {
                                console.error(
                                    "Erreur création table country_names:",
                                    err.message,
                                );
                                reject(err);
                                return;
                            ...
                            db.run(
                                "CREATE INDEX IF NOT EXISTS idx_country_names ON country_names(country, annais)",
                                (err) => {
                                    if (err) {
                                        console.error(
                                            "Erreur création index country_names:",
                                            err.message,
                                        );
                                        reject(err);
                                        return;
                                    ...
                                    db.run("BEGIN TRANSACTION", (err) => {
                                        if (err) {
                                            console.error(
                                                "Erreur début transaction country_names:",
                                                err.message,
                                            );
                                            reject(err);
                                            return;
                                        ...
                                        for (
                                            let i = 0;
                                            i < countryBatch.length;
                                            i += batchSize
                                        ) {
                                            const batch = countryBatch.slice(
                                                i,
                                                i + batchSize,
                                            );
                                            ...
                                            db.run(
                                                ...
                                                (err) => {
                                                    if (err) {
                                                        console.error(
                                                            "Erreur insertion batch country_names:",
                                                            err.message,
                                                        );
                                                    }
                                                },
                                        ...
                                        db.run("COMMIT", (err) => {
                                            if (err) {
                                                ...
                                                console.log(
                                                    `Importation de ${countryRows} lignes dans country_names terminée`,
                                                );
                                                resolve();
                                            }
                                        });
    ...
    function importDepartmentNames() {
        ...
        function readDepartmentNames() {
            return new Promise((resolve, reject) => {
                if (!fs.existsSync("setup/analyse_prenom_departement.csv")) {
                    console.error(
                        "Erreur: analyse_prenom_departement.csv n'existe pas dans le répertoire courant",
                    );
                    resolve(); // Continue with empty data
                    return;
                }

                fs.createReadStream("setup/analyse_prenom_departement.csv")
                    .pipe(csv())
                    .on("data", (row) => {
                        const missingFields = [];
                        ...
                        if (missingFields.length > 0) {
                            console.warn(
                                `Ligne ignorée dans analyse_prenom_departement.csv (champs manquants: ${missingFields.join(", ")}):`,
                                row,
                            );
                            return;
                        }
                        ...
                        if (
                            ...
                            console.warn(
                                `Code département invalide ignoré: ${dpt}`,
                                row,
                            );
                            return;
                        }
                        ...
                        if (dpt === "20") {
                            ...
                            console.log(
                                `Stored Corse (20) data for 2A and 2B:`,
                                { annais, musulman_pct },
                            );
                        }
                    })
                    .on("end", () => {
                        console.log(
                            `Lecture de analyse_prenom_departement.csv terminée: ${departmentRows} lignes`,
                        );
                        if (departmentRows === 0) {
                            console.warn(
                                "Avertissement: analyse_prenom_departement.csv est vide ou n'a pas de données valides",
                            );
                        }
                        // Add 2A and 2B data if not already present
                        if (corseData.length > 0) {
                            console.log(
                                `Adding ${corseData.length} entries for 2A and 2B from dpt 20`,
                            );
                    ...
                    .on("error", (err) => {
                        console.error(
                            "Erreur lecture analyse_prenom_departement.csv:",
                            err.message,
                        );
                        reject(err);
                    });
                ...
                db.serialize(() => {
                    ...
                    `,
                        (err) => {
                            if (err) {
                                console.error(
                                    "Erreur création table department_names:",
                                    err.message,
                                );
                                reject(err);
                                return;
                            ...
                            db.run(
                                "CREATE INDEX IF NOT EXISTS idx_department_names ON department_names(dpt, annais)",
                                (err) => {
                                    if (err) {
                                        console.error(
                                            "Erreur création index department_names:",
                                            err.message,
                                        );
                                        reject(err);
                                        return;
                                    ...
                                    db.run("BEGIN TRANSACTION", (err) => {
                                        if (err) {
                                            console.error(
                                                "Erreur début transaction department_names:",
                                                err.message,
                                            );
                                            reject(err);
                                            return;
                                        ...
                                        for (
                                            ...
                                            db.run(
                                                ...
                                                (err) => {
                                                    if (err) {
                                                        console.error(
                                                            "Erreur insertion batch department_names:",
                                                            err.message,
                                                        );
                                                    }
                                                },
                                        ...
                                        db.run("COMMIT", (err) => {
                                            if (err) {
                                                console.error(
                                                    "Erreur commit department_names:",
                                                    err.message,
                                                );
                                                db.run("ROLLBACK");
                                                reject(err);
                                                ...
                                                console.log(
                                                    `Importation de ${departmentRows} lignes dans department_names terminée`,
                                                );
                                                resolve();
                                            }
                                        });
        ...
        function readCommuneNames() {
            return new Promise((resolve, reject) => {
                if (!fs.existsSync("setup/analyse_prenom_commune.csv")) {
                    console.error(
                        "Erreur: setup/analyse_prenom_commune.csv n'existe pas dans le répertoire courant",
                    );
                    resolve(); // Continue with empty data
                    return;
                }
                ...
                fs.createReadStream("setup/analyse_prenom_commune.csv")
                    ...
                    .on("data", (row) => {
                        ...
                        if (missingFields.length > 0) {
                            console.warn(
                                `Ligne ignorée dans analyse_prenom_commune.csv (champs manquants: ${missingFields.join(", ")}):`,
                                row,
                            );
                            return;
                        }
                    ...
                    .on("end", () => {
                        console.log(
                            `Lecture de analyse_prenom_commune.csv terminée: ${communeRows} lignes`,
                        );
                        if (communeRows === 0) {
                            console.warn(
                                "Avertissement: analyse_prenom_commune.csv est vide ou n'a pas de données valides",
                            );
                        }
                        resolve();
                    })
                    .on("error", (err) => {
                        console.error(
                            "Erreur lecture analyse_prenom_commune.csv:",
                            err.message,
                        );
                        reject(err);
                    });
                ...
                db.serialize(() => {
                    ...
                    `,
                        (err) => {
                            if (err) {
                                console.error(
                                    "Erreur création table commune_names:",
                                    err.message,
                                );
                                reject(err);
                                return;
                            ...
                            db.run(
                                "CREATE INDEX IF NOT EXISTS idx_commune_names ON commune_names(COG, annais)",
                                (err) => {
                                    if (err) {
                                        console.error(
                                            "Erreur création index commune_names:",
                                            err.message,
                                        );
                                        reject(err);
                                        return;
                                    ...
                                    db.run("BEGIN TRANSACTION", (err) => {
                                        if (err) {
                                            console.error(
                                                "Erreur début transaction commune_names:",
                                                err.message,
                                            );
                                            reject(err);
                                            return;
                                        ...
                                        for (
                                            let i = 0;
                                            i < communeBatch.length;
                                            i += batchSize
                                        ) {
                                            const batch = communeBatch.slice(
                                                i,
                                                i + batchSize,
                                            );
                                            const placeholders = batch
                                            ...
                                            db.run(
                                                ...
                                                (err) => {
                                                    if (err) {
                                                        console.error(
                                                            "Erreur insertion batch commune_names:",
                                                            err.message,
                                                        );
                                                    }
                                                },
                                        ...
                                        db.run("COMMIT", (err) => {
                                            if (err) {
                                                ...
                                                console.log(
                                                    `Importation de ${communeRows} lignes dans commune_names terminée`,
                                                );
                                                resolve();
                                            }
                                        });
